...
def test_design_doc_delete(setup_client_syncgateway_test):
    ...
    5. delete design doc to version 1 to fetch doc._id, doc._rev for docs with content.
    6. Create design doc version 2 with same name as design doc version 1 to fetch doc._id,
       doc._rev for docs with no content. 
    7. Run a query and check for 5 expected docs with design doc version 2
    -> With CBL 1.4.0 or earlier, even though design doc is deleted and new design doc with same name version 2,
       the second query will use design doc 1 and return 10 docs instead of 5
    """
    ...
    # deleting the design doc
    dd_rev = client.get_design_doc_rev(url=ls_url, db=ls_db, name=d_doc_name)
    assert dd_rev and dd_rev.startswith("1-")
    log_info("dd_rev: {}".format(dd_rev))
    log_info("Deleting design doc - dd")
    client.delete_design_doc(url=ls_url, db=ls_db, name=d_doc_name, rev_id=dd_rev)

    # verifying that we can't view query for deleted design doc
    try:
