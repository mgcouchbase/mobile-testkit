...
def verify_docs_present_listener(expected_docs, actual_docs):
    missing_docs = []
    ...
    for actual_doc in actual_docs["rows"]:
        ...
        elif actual_doc["value"]["rev"] != expected_docs[actual_doc["id"]]:
...
def verify_docs_present_sync_gateway(expected_docs, actual_docs):
    missing_docs = []
    ...
    for actual_doc in actual_docs["rows"]:
        if "error" in actual_doc or ("value" in actual_doc and len(actual_doc["value"]) == 0):
            # Doc not found
            missing_docs.append(actual_doc)
        elif actual_doc["_rev"] != expected_docs[actual_doc["_id"]]:
            # Found the doc but unexpected rev on LiteServ
            missing_docs.append(actual_doc)
